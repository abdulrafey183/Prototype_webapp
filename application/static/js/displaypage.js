...
function make_plot_card(plot) {
  ...
  if (plot.deal)
    str +=
      "<tr class='text-dark-3'><th scope='row'>Deal</th><td><a href='/deal/" +
      plot.deal.id +
      "'>" +
      'Deal# ' +
      plot.deal.id +
  ...
  str +=
    "<tr class='text-dark-3'><td scope='row'>" +
    "<span><a href='/plot/" +
    plot.id +
    "''>Show Details</a></span></td><td></td>" +
    '</tr>';
...
function make_buyer_card(buyer) {
  ...
  if (deal_ids != 'None') {
    str +=
      '<tr class="text-dark-3"><th scope="row">Deals</th><td>' +
      deal_ids +
      '</td></tr>';
  }
  str +=
    '</tbody></table></div></div></div></section>' + '</div></div></div></div>';
...
function make_deal_card(deal) {
  let str =
    ...
    deal.id +
    "' class='collapse' aria-labelledby='headingOne' data-parent='#accordionExample'><div class='card-body'>" +
    "<section class='my-3'><div class='container'><div class='row'><div class='col-12'><table class='table mt-4'><tbody><tr class='text-dark-3'><td scope='row'><a href='/add/transaction/receivepayment/" +
    deal.id +
    "'>Recieve Payment</a></td><td>" +
    '</td></tr>' +
...
function make_CA_card(CA) {
  ...
  if (CA.deals != null)
    for (var deal of CA.deals) {
      deal_ids += ' <a href="/deal/' + deal.id + '">' + deal.id + '</a>';
    }
  else deal_ids = 'None';
...
function make_ET_card(ET) {
  let str =
    ...
    ET.id +
    "'>Show Details</a></div></div></div></section>" +
    '</div></div></div></div>';
...
function inject_div(name, list) {
  ...
  for (let element of list) {
    ...
    if (name == 'deal') getbuyer(element.buyer_id);
  }
}
